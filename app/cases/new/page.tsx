...
export default function NewCasePage() {
  ...
  // After case creation — holds created case IDs
  ...
  // Design flow: create case → go to upload step
  ...
  // Print flow: create case(s) → go to upload step
  const handleCreateBatch = async () => {
    ...
    try {
      ...
      if (ids.length === 1) {
        setCreatedCaseId(ids[0]);
      } else {
        setCreatedBatchIds(ids);
      }
      setStep(2);
    } catch (e) {
  ...
  const handleProceed = () => {
    if (createdCaseId) {
      if (workflow === "design") {
        router.push(`/cases/${createdCaseId}/treatment`);
      } else {
        router.push(`/cases/${createdCaseId}/prepare`);
      }
    } else if (createdBatchIds.length > 0) {
      // For batch, go to first case's prepare page
      router.push(`/cases/${createdBatchIds[0]}/prepare`);
    }
  };

  // For batch upload step: which case is currently being uploaded to
  const [activeBatchIndex, setActiveBatchIndex] = useState(0);
  const [batchUploads, setBatchUploads] = useState<Record<string, Scan[]>>({});

  const handleBatchUploadComplete = (caseId: string, scans: Scan[]) => {
    setBatchUploads((prev) => ({
      ...prev,
      [caseId]: [...(prev[caseId] || []), ...scans],
    }));
  };

  const totalBatchUploads = Object.values(batchUploads).reduce((sum, s) => sum + s.length, 0);
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              ...
              <div className="mt-4 flex items-center gap-2">
                <SmallBadge text="Supports batch printing" />
              </div>
            </button>
            ...
            >
              ...
              <div className="mt-4 flex items-center gap-2">
                <SmallBadge text="Includes segmentation" />
              </div>
            </button>
          </div>
        </div>
      ...
      {/* Step 1: Patient Details - Print (single or batch) */}
            ...
            <div className="mt-4 p-3 rounded-lg bg-primary/5 border border-primary/20">
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground">
                  {batchEntries.filter((e) => e.patientName.trim()).length} patient{batchEntries.filter((e) => e.patientName.trim()).length !== 1 ? "s" : ""}. Upload scans for each in the next step.
                </span>
              </div>
            </div>
          )}
      ...
      {/* Step 2: Upload Files - Single Case */}
      {step === 2 && createdCaseId && createdBatchIds.length === 0 && (
        <div>
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-1">
              <CheckCircle2 className="w-5 h-5 text-success" />
              <h1 className="text-2xl font-bold">Upload Scans</h1>
            </div>
            <p className="text-muted-foreground text-sm">
              Case created for <span className="text-foreground font-medium">{patientName || batchEntries[0]?.patientName}</span>. Now upload your STL files.
            </p>
          </div>

          <ScanUploader caseId={createdCaseId} onUploadComplete={handleUploadComplete} />

          {uploadedScans.length > 0 && (
            <div className="mt-4 space-y-2">
              <h3 className="text-sm font-medium text-muted-foreground">
                {uploadedScans.length} file{uploadedScans.length !== 1 ? "s" : ""} uploaded
              </h3>
              {uploadedScans.map((scan) => (
                <div key={scan.id} className="flex items-center gap-3 p-2 rounded-lg bg-secondary">
                  ...
                  <span className="text-xs text-muted-foreground">{(scan.fileSize / 1024).toFixed(0)} KB</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-3 mt-6">
            ...
            <Button
              ...
              disabled={uploadedScans.length === 0}
            >
              {workflow === "design" ? (
                <>Segment Teeth <ArrowRight className="w-4 h-4" /></>
              ) : (
                <>Prepare Print <ArrowRight className="w-4 h-4" /></>
              )}
            </Button>
          </div>
          {uploadedScans.length === 0 && (
            <p className="text-xs text-muted-foreground mt-2">Upload at least one STL file to continue.</p>
          )}
        </div>
      )}

      {/* Step 2: Upload Files - Batch Cases */}
      {step === 2 && createdBatchIds.length > 1 && (
        <div>
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-1">
              <CheckCircle2 className="w-5 h-5 text-success" />
            ...
            <p className="text-muted-foreground text-sm">
              {createdBatchIds.length} cases created. Upload STL files for each patient.
            </p>
          </div>

          {/* Patient tabs */}
          <div className="flex gap-1 border-b border-border mb-4">
            {createdBatchIds.map((id, idx) => {
              const entry = batchEntries.filter((e) => e.patientName.trim())[idx];
              const scansForCase = batchUploads[id] || [];
              return (
                <button
                  key={id}
                ...
                >
                  {entry?.patientName || `Patient ${idx + 1}`}
                  {scansForCase.length > 0 && (
          ...
          {/* Uploader for active patient */}
          {createdBatchIds[activeBatchIndex] && (
            <div>
              <ScanUploader
                key={createdBatchIds[activeBatchIndex]}
                caseId={createdBatchIds[activeBatchIndex]}
                onUploadComplete={(scans) => handleBatchUploadComplete(createdBatchIds[activeBatchIndex], scans)}
              />

              {(batchUploads[createdBatchIds[activeBatchIndex]] || []).length > 0 && (
                <div className="mt-3 space-y-1">
                  {(batchUploads[createdBatchIds[activeBatchIndex]] || []).map((scan) => (
                    <div key={scan.id} className="flex items-center gap-3 p-2 rounded-lg bg-secondary">
                      <File className="w-4 h-4 text-muted-foreground" />
                      <span className="text-sm flex-1 truncate">{scan.originalName}</span>
                      <span className="text-xs text-muted-foreground">{(scan.fileSize / 1024).toFixed(0)} KB</span>
          ...
          <div className="flex gap-3 mt-6">
            ...
            <Button
              onClick={() => router.push(`/cases/${createdBatchIds[0]}/prepare`)}
              disabled={totalBatchUploads === 0}
            >
              Prepare Print <ArrowRight className="w-4 h-4" />
            </Button>
          </div>
